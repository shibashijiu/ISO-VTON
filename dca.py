...
import einops
from DCA.utils.main_blocks import *
from DCA.utils.dca_utils import *
...
class DCA(nn.Module):
    ...
    def forward(self, raw):
        #print("Input shape:", [r.shape for r in raw])  # 打印原始输入尺寸

        x = self.m_apply(raw, self.patch_avg)
        #print("After patch_avg shape:", [xi.shape for xi in x])  # 打印经过 patch_avg 后的尺寸

        x = self.m_apply(x, self.avg_map)
        #print("After avg_map shape:", [xi.shape for xi in x])  # 打印经过 avg_map 后的尺寸

        for block in self.attention:
            x = block(x)
            #print("After attention block shape:", [xi.shape for xi in x])  # 打印每个 attention block 后的尺寸

        x = [self.reshape(i) for i in x]
        #print("After reshape shape:", [xi.shape for xi in x])  # 打印经过 reshape 后的尺寸

        x = self.m_apply(x, self.upconvs)
        #print("After upconvs shape:", [xi.shape for xi in x])  # 打印经过 upconvs 后的尺寸
        ...
        '''
        #print("After m_sum shape:", [xo.shape for xo in x_out])  # 打印经过 m_sum 后的尺寸

        x_out = self.m_apply(x_out, self.bn_relu)
        #print("Output shape:", [xo.shape for xo in x_out])  # 打印最终输出尺寸
    ...
    def m_sum(self, x, y):
        assert isinstance(x, list) and isinstance(y, list), "x and y should be lists."
        ...
        for xi, yi in zip(x, y):
            if xi.shape != yi.shape:
                # 选择一个目标尺寸，例如 x 的尺寸
                target_size = yi.shape[2:]

                # 调整 y 到目标尺寸
                xi = torch.nn.functional.interpolate(xi, size=target_size, mode='bilinear', align_corners=False)
